...
import { Check, User, Mail, Phone, Users, ChevronRight, Info } from "lucide-react";
...
export default function AttendeeForm({
  ...
  slots: Slot[];
  onSubmit: (attendees: Record<number, Attendee>, sharedTableStatus: Record<string, boolean>) => void;
}) {
  const [attendees, setAttendees] = useState<Record<number, Attendee>>({});
  const [useSameTableDetails, setUseSameTableDetails] = useState<Record<string, boolean>>({});
  ...
  const groupedSlots: { type: 'single' | 'table'; tableId?: string; slots: { slot: Slot; index: number }[] }[] = [];
  ...
  const isFormValid = slots.every((_, i) => 
  ...
  return (
    <div className="space-y-8 max-w-2xl mx-auto pb-20">
      <div className="space-y-2">
        <h2 className="text-3xl font-black text-white tracking-tight">Attendee Details</h2>
        <p className="text-white/50 text-sm">Please provide information for each ticket holder.</p>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <label className="flex items-center gap-3 p-4 rounded-xl border border-white/10 bg-white/[0.03] cursor-pointer hover:bg-white/[0.05] transition-colors">
          ...
          />
          <span className="text-sm font-medium text-white/80">Use buyer details for first attendee</span>
        </label>
          ...
          <div key={groupIndex} className="space-y-4">
            {group.type === 'table' ? (
              <div className="p-6 rounded-2xl border border-white/10 bg-white/[0.02] space-y-6 relative overflow-hidden">
                <div className="absolute top-0 left-0 w-1 h-full bg-ec4899" style={{ backgroundColor: '#ec4899' }} />
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-white text-lg">{group.slots[0].slot.label.split(' — ')[0]}</h3>
                      <p className="text-[10px] text-white/40 uppercase tracking-widest font-bold">Table Reservation</p>
                    </div>
                  </div>
                  
                  <label className="flex items-center gap-2 cursor-pointer">
                    <span className="text-[11px] font-bold text-white/50 uppercase tracking-wider">Same for all seats</span>
                    <div 
                      onClick={() => group.tableId && handleToggleSameTable(group.tableId, group.slots[0].index)}
                    ...
                    >
                      <div className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all duration-300 ${useSameTableDetails[group.tableId!] ? 'left-6' : 'left-1'}`} />
                    </div>
                  </label>
                </div>
                ...
                <div className="space-y-6">
                  {group.slots.map(({ slot, index }, i) => {
                    ...
                    if (useSameTableDetails[group.tableId!] && i > 0) return null;

                    return (
                      <div key={index} className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-500">
                        <p className="text-xs font-bold text-white/30 uppercase tracking-widest">
                          {useSameTableDetails[group.tableId!] ? "Table Guest Details" : `Seat ${i + 1}`}
                        </p>
                        <AttendeeInputs 
                          index={index} 
                          values={attendees[index] || { name: "", email: "", phone: "" }} 
                          onChange={update} 
                        />
                      </div>
                    );
              ...
              <div className="p-6 rounded-2xl border border-white/10 bg-white/[0.02] space-y-4 relative overflow-hidden">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-bold text-white text-base">{group.slots[0].slot.label}</h3>
                </div>
                <AttendeeInputs 
                  index={group.slots[0].index} 
                  values={attendees[group.slots[0].index] || { name: "", email: "", phone: "" }} 
                  onChange={update} 
                />
              </div>
            )}
      ...
      <div className="flex flex-col gap-4">
        <button
          ...
          className="w-full bg-primary hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed text-white py-5 rounded-2xl font-black tracking-widest text-sm transition-all duration-300 shadow-[0_8px_30px_rgba(59,130,246,0.3)] flex items-center justify-center gap-2 group"
        >
